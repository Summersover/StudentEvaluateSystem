...
import {getResSitu, updateResSitu, deleteResSitu} from "@/api/updateResSitu";
import FileUpload from '@/views/user/student/StudentFileUpload.vue'; 
export default {
  name: "update",
  components: {
    FileUpload
  ...
  methods: {
    ...
    handleAddRow() {
      const newRow = { id: ++this.idSeed, rank: '', place: '', prize: '', editable: true, isNew: true }
    ...
    handleDelete(row) {
      this.$confirm("确认删除该行数据?", "提示", {
        ...
        center: true,
          ...
          .then(() => {
            if (row.editable) {
              deleteResSitu([row], this.username) // 只删除正在编辑的行数据
                  .then(() => {
                    console.log("删除成功！");
    ...
    handleSubmit() {
      this.$confirm('是否确认提交当前填写内容?', '提示', {
        ...
        updateResSitu(editableRows, this.username)
            .then(res => {
              if (res.code === 200) {
                console.log("success!")
